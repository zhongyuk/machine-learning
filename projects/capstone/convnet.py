...
def convnet_model(data, weights, baises, dropout=True, keep_prob=.5):
    ...
    conv = conv_layer(pool, weights['conv2_wt'], biases['conv2_bi'], padding="VALID")
    pool = pool_layer(conv, 'max')
    ...
    output = full_layer(fc, weights['fc3_wt'], biases['fc3_bi'], False)
    return output
...
if __name__=='__main__': 
	...
	convnet_shapes = {'conv1' : [kernel_size5, kernel_size5, num_channels, num_filter],
			  ...
			  'fc1'   : [((image_size/2-kernel_size3+1)/2)**2*num_filter, fc_size1],
			  'fc2'   : [fc_size1, fc_size2],
	...
	weights['fc3_wt'],   biases['fc3_bi']   = initialize_variable(convnet_shapes['fc3'])
        ...
        learning_rate = tf.train.exponential_decay(.00001, global_step, 10000, .7, staircase=True)
